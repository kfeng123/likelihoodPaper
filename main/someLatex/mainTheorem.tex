...
\begin{enumerate}[(i)]
    \item
        \textbf{Assumtpion 1} The assumptions of Bernstein-von Mises Theorem (see \cite{van2000asymptotic} Chapter 10) are met:  $\Theta$ is a subset of $\mathbb{R}^p$. 试验($P_{\theta}:\theta\in\Theta$)在$\theta_0\in \Theta$处均方可微，具有非奇异信息阵$I_{\theta_0}$，假设对任意的$\epsilon>0$，存在一个相合检验序列:
        \begin{equation}
