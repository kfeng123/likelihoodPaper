...
For $t>0$, we say $L_t(\cdot;\BX^{n})$ is $\sqrt{n}$-consistent if for every $M_n\to \infty$,
    ...
    \begin{proposition}
        Suppose that Assumption~\ref{Assumption1} holds, $L_{t}(\cdot;\BX^{n})$, $L_{t}^{(0)}(\cdot;\BX^{n})$ are $\sqrt{n}$-consistent, $\pi(\theta)$ is continuous at $\theta_0$ with $\pi(\theta_0)>0$, $\pi_0(\nu)$ is continuous at $\nu_0$ with $\pi_0(\nu_0)>0$.
        Then,
\begin{align*}
...
\begin{proposition}
    Suppose that Assumptions \ref{Assumption1}, \ref{assumption2019} hold, $\pi(\theta)$ is countinuous at $\theta_0$ with $\pi(\theta_0) > 0 $, $t \to 0$, $tn \to \infty$.
    Suppose
...
\begin{equation}\label{eq:leiApp}
    ...
    \end{aligned}
\end{equation}
To prove the proposition, we only need to proof
\begin{align}
...
Fix a $t_0 \in [0,1)$, we claim that
\begin{align}
    ...
    \label{eq:fort0}
\end{align}
In fact, Assumption \ref{assumption2019} implies that
\begin{align*}
