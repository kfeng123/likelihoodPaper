...
Here $m_0$ is the minimal training sample size.
    ...
    Let $R_n (\theta\| \theta' )= \log \left\{ p_n(\BX^{n}|\theta)/ p_n(\BX^{n}|\theta') \right\}$ denote the log-likelihood ratio between $p_n(\BX^{n} | \theta )$ and $ p_n(\BX^{n} | \theta' )$.
       Let
       \begin{align*}
           \pi_t( \theta | \BX^{n})
           =
...
Let $\BX^{n}=(X_1,\ldots,X_n)$ be independent identically distributed (iid) observations taking values in some measurable space $(\mathcal{X};\mathscr{A})$.
...
A central tool in Bayesian hypothesis testing framework is Bayes factor
\begin{equation*}
    \text{BF}(\BX^{n})=
...
For every $M>0$,
    ...
    \end{equation*}
    where $\Delta_{n,\theta_0}=n^{-1/2}\sum_{i=1}^n I(\theta_0)^{-1}\dot{\ell}_{\theta_0}(X_i)$.
\end{assumption}     
...
From Proposition \eqref{prop:expansion}, the asymptotic distribution of $\log \textrm{BF}_t (\BX^n)$ is free of the nuisance parameter $\nu$ if and only if
...
Several variants of Bayes factor have been proposed to solve this problem.
...
$$
    \Lambda_{a,b}=
    \frac{L_{a}(\Theta;\BX^{n})}{L_{b}(\Theta;\BX^{n})}\cdot \frac{L_{b}^{(0)}(\tilde{\Theta}_0;\BX^{n})}{L_{a}^{(0)}(\tilde{\Theta}_0;\BX^{n})}.
$$
