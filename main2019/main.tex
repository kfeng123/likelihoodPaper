...
Instead, we shall utilize the $\sqrt n$-consistency of the posterior density $\pi(\theta | \BX^n) $.
We say $\pi(\theta | \BX^n)$ is $\sqrt n$-consistent if for every $M_n \to \infty$,
\begin{align*}
...
We denote by
$L_t(\BX^{n})=\int_{\Theta}\big[p_n(\BX^{n}|\theta)\big]^t \pi(\theta)\, \mathrm d\theta$
the power marginal likelihood with power $t>0$, and similarly define $L_t^{(0)}(\BX^{n})$.
For $t > 0$, let $\text{BF}_t (\BX^n) = L_t(\BX^n)/ L_t^{(0)} (\BX^n)$.
Then $\text{BF}_1(\BX^n)$ is the usual Bayes factor.
It can be expected that $\text{BF}_t (\BX^n)$ depends on the nuisance parameters and the priors in the same way for all $t>0$.
...
\Lambda_{a,b} (\BX^n) =
    ...
    In what follows, we shall derive the asymptotic distribution of $\Lambda_{a,b}(\BX^n)$ in three settings.
    In all three settings, $a$ is fixed.
    In the first setting, $b$ is also fixed.
...
\begin{assumption}
    We assume $\pi(\theta)$ is continuous at $\theta_0$ with $\pi(\theta_0)>0$, $\pi_0(\nu)$ is continuous at $\nu_0$ with $\pi_0 (\nu_0)> 0$.
    For every $t^* \in (0,1)$, there exist $c^*, c^\dagger > 0$, $t_0^* \in (0,1)$ and $c_0^*, c_0^\dagger > 0$,
    \begin{align*}
...
.
        $$
    \item
        Suppose $a>b>0$, $a$ is fixed and as $n \to \infty$, $b\to 0$, $nb\to b^* \in (c^*, +\infty)$.
        Then
\begin{align*}
    2\log \Lambda_{a,b}
...
\end{align*}
\item
    Suppose $a>b>0$, $a$ is fixed and as $n \to \infty$, $b\to 0$, $bn \to \infty$.
    Then
\begin{align*}
    2\log \Lambda_{a,b}
    +
...
It can be seen that when $nb \to \infty$, $\Lambda_{a,b}$ has Wilks phenomenon for any prior distributions.
To formulate a test with asymptotic type I error rate $\alpha$, the critical value of $2\log \Lambda_{a,b}$ can be defined to be $-(p-p_0)\log (1+a/b)+ a\chi^2_{\alpha}(p-p_0)$ where $\chi^2_{\alpha}(p-p_0)$ is the $1-\alpha$ quantile of a chi-squared random variable with $p-p_0$ degrees of freedom.
...
Surprisingly, $\Lambda_{a,b}$ can be easily computed provided sampling from the posterior distribution is feasible.
The computation of the ILRT statistic is relatively simple.
It can be seen that $\Lambda_{a,b}$ can be written as
\begin{equation*}
    \Lambda_{a,b}
    =
    \frac{\int_{\Theta} \big[p_n(\BX^{n}|\theta)\big]^{a-b}\pi_b(\theta|\BX^{n})\,d\theta}{\int_{\tilde{\Theta}_0} \big[p_n(\BX^{n}|\nu,\xi_0)\big]^{a-b}\pi_{b,0}(\nu|\BX^{n})\,d\nu}.
\end{equation*}
We can independently generate $\theta_1,\ldots,\theta_m$ and $\nu_1,\ldots,\nu_m$ according to $\pi_b(\theta|\BX^{n})$ and $\pi_{b,0}(\nu;\BX^{n})$ for a large $m$.
...
.
$$
Furthermore, assume that for every $\epsilon>0$, there exist Lebesgue integrable functions $T(h)$ and $T^{(0)}(h)$ such that 

    \begin{equation}\label{Assump31}
        \liminf_{n\to \infty}P_{\theta_0}^n\left\{\sup_{h\in \mathbb{R}^p}(\pi_n(h;\BX^{n})-T(h))\leq 0\right\}\geq 1-\epsilon.
\end{equation}
    \begin{equation}\label{Assump31l}
        \liminf_{n\to \infty}P_{\theta_0}^n\left\{\sup_{h^{(0)}\in \mathbb{R}^{p_0}}(\pi_n(h^{(0)};\BX^{n})-T^{(0)}(h^{(0)}))\leq 0\right\}\geq 1-\epsilon.
\end{equation}


\end{assumption}
The conditions~\eqref{Assump31} and~\eqref{Assump31l} assume that there is a function controlling the tail of the weight functions.
...
\begin{proposition}\label{exponentialCon}
    ...
    $I(\theta_0)$ is positive-definite.
    Suppose the prior density $\pi(\theta)$ is continuous at $\theta_0$ with $\pi(\theta_0) > 0$, and there exist $c^* > 0$ such that
    \begin{align*}
        \int_{\Theta} 
        ...
        .
    \end{align*}
    Then $L_{t}(\cdot;\BX^{n})$ is consistent for any fixed $t>0$.
\end{proposition}
...
For general models, a fundamental question is whether the fractional posterior is always well defined with proper prior.
...
\begin{assumption}\label{Assumption4}
    For some $\alpha\in(0,1)$, there exist positive constants $\delta$, $\epsilon$ and $C$ such that,
     $D_{\alpha}(\theta_0||\theta)  \geq  C \|\theta-\theta_0\|^2$ for $\|\theta-\theta_0\|\leq \delta$ and $D_{\alpha}(\theta_0||\theta) \geq \epsilon$ for $\|\theta-\theta_0\|>\delta$.
\end{assumption}
\begin{remark}
...
\begin{proposition}\label{Theoremless1}
    Suppose $\theta_0$ is an interior of $\Theta$, $\pi(\theta)$ is continuous at $\theta_0$ and $\pi(\theta_0)>0$.
    Under Assumptions \ref{Assumption1} and~\ref{assumption2019}, for fixed $t\in(0,1)$, $L_t(\cdot;\BX^{n})$ is consistent.
\end{proposition}
...
\log \Lambda_{a,b}
    ...
    +o_{P^n_{\theta_n}}(1).
        \end{align*}
        Then the assertion (a) follows from Lemma \ref{lemma:score}.

        Similarly, assertion (b) and (b) follows from Proposition \ref{prop:tto0} and Lemma \ref{lemma:score}.
...
\exp\left\{ -t R_n (\theta_0 \| \theta) \right\}
    ...
    (1+o_{P_{\theta_0}^n}(1))
     \pi(\theta_0) n^{-p/2} 
     \int_{\{h:h\leq M_n\}} \exp\left\{ t h^\top  I(\theta_0) \Delta_{n,\theta_0}-\frac{t}{2}h^\top  I(\theta_0) h\right\} \, \mathrm d h
    \\
    =&
    (1+o_{P_{\theta_0}^n}(1))
     \pi(\theta_0) n^{-p/2} 
    \int_{\mathbb{R}^p} \exp\left\{ t h^\top  I(\theta_0)\Delta_{n,\theta_0}-\frac{t}{2}h^\top  I(\theta_0) h\right\} \, \mathrm d h
    \\
    =&
    (1+o_{P^n_{\theta_0}}(1))
        \pi(\theta_0)
...
$ for $\|\theta-\theta_0\|\geq M_n/\sqrt{n}$.
    ...
    \begin{equation*}
     ...
     .
        \end{split}
    \end{equation*}
    The last two inequalities imply that with probability tending to $1$,
    $$
    \begin{aligned}
        R_n(\theta_0 \| \theta)
        &\geq
...
\begin{align*}
    &P_{\theta_0}^n\int_{\{\theta:\|\theta-\theta_0\|\geq {M_n}/{\sqrt{n}}\} } \exp\left\{  -t R_n (\theta_0 \| \theta) \right\} \pi(\theta) \, \mathrm d \theta
    \\
    =& 
    \int_{\{\theta:\|\theta-\theta_0\|\geq {M_n}/{\sqrt{n}}\} } \exp \left\{-t n D_{1-t}(\theta_0||\theta) \right\} \pi(\theta) \, \mathrm d \theta.
\end{align*}
Assumption \ref{assumption2019} implies that there exist $\delta >0$, $C >0$ and $\epsilon > 0$ such that $D_{1-t}(\theta_0 \| \theta) \geq C \| \theta - \theta_0\|^2 $ for $\|\theta - \theta_0\| \leq \delta$ and $D_{1-t}(\theta_0 \| \theta)\geq \epsilon$ for $\|\theta - \theta_0\| >\delta$.
...
\begin{equation}\label{Prop4:eq2}
    n^{p/2}\int_{\{\theta:\|\theta-\theta_0\|\geq {M_n}/{\sqrt{n}}\} } \Big[ \frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}  \Big]^{t} \pi(\theta) \, \mathrm d \theta
    =o_{P^n_{\theta_0}}(1).
\end{equation}
    ...
    $$
    \begin{aligned}
        & \int_{\Theta}\Big[\frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\Big]^{t} \pi(\theta)\, \mathrm d\theta
        \geq
        \int_{\{\theta:\|\theta-\theta_0\|\leq n^{-1/2}\}}\Big[\frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\Big]^{t} \pi(\theta)\, \mathrm d\theta
        \\
        \geq &
        \Big(
...
\Big[\frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\Big]^{t} \pi(\theta)
        \Big)
        \int_{\{\theta:\|\theta-\theta_0\|\leq n^{-1/2}\}}1\, \mathrm d\theta\\
        \geq&
        \Big(
        \exp
\Big[
   ...
   \end{aligned}
    ...
    $$
         \int_{\Theta}\Big[\frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\Big]^{t} \pi(\theta)\, \mathrm d\theta\geq c n^{-p/2}.
    $$
...
\begin{proof}[\textbf{Proof of Theorem~\ref{theoremMain}}]
    ...
    \begin{equation*}
        \Lambda_{a,b}^*=\frac{
    \int \left[{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}/{p_n(\BX^{n}|\theta_0)}\right]^a \pi_n (h;\BX^{n}) \, \mathrm dh
        }{
    \int \left[ {p_n(\BX^{n}|\nu_0+n^{-1/2}h^{(0)},\xi_0)}/{p_n(\BX^{n}|\theta_0)}\right]^a \pi_n (h^{(0)};\BX^{n}) \, \mathrm dh^{(0)}
}.
    \end{equation*}
...
\begin{equation}\label{eq:8}
    \begin{split}
    &\int_{\|h\|\leq M} \left[\frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}\right]^a \pi_n (h;\BX^{n}) \, \mathrm dh\\
    =&
    \exp [o_{p^n_0}(1)]\int_{\|h\|\leq M} \exp\big[ a h^\top  I_{\theta_0}\Delta_{n,\theta_0}- \frac{a}{2}h^\top  I_{\theta_0}h\big]\pi_n (h;\BX^{n}) \, \mathrm dh
    \\
    =&\int_{\|h\|\leq M} \exp\big[ a h^\top  I_{\theta_0}\Delta_{n,\theta_0}-\frac{a}{2}h^\top  I_{\theta_0}h\big] \phi(h;\Delta_{n,\theta_0}, b^{-1} I_{\theta_0}^{-1})\, \mathrm dh
    +o_{P^n_{\theta_0}}(1),
\end{split}
\end{equation}
...
\begin{equation}\label{eq:4}
    \int_{\|h\|> M_n}\left[\frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}\right]^{a} \pi_n(h;\BX^{n})\, \mathrm dh
...
\begin{equation}\label{eq:4l}
    ...
    \\
    \leq&
    \int_{\|h\|> M_n}\left[\frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}\right]^{a} T(h)\, \mathrm dh.
    \end{split}
\end{equation}
...
\begin{equation*}
    ...
    \\
    =&
    \int p_n(\BX^{n}|\theta_0+n^{-1/2}h)^a p_n(\BX^{n}|\theta_0)^{1-a} \, \mathrm d\mu^n
    \\
    \leq& 1.
    \end{split}
\end{equation*}
...
\begin{equation*}
   P^n_{\theta_0} \int_{\|h\|> M_n}\left[\frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}\right]^{a} T(h)\,  \mathrm dh
   \leq
...
\begin{equation*}
    \begin{split}
    &\int \left[ \frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}\right]^a \pi_n (h;\BX^{n})  \, \mathrm dh
    \\
...
We shall verify Assumption~\ref{Assumption1} and Assumption~\ref{assumption2019}.
