...
It can be seen that when $nb \to \infty$, $\Lambda_{a,b}$ has Wilks phenomenon for any prior distributions so long as the prior density on the true value is positive.
...
\end{proposition}
\begin{proof}
    Assertion (a) follows from the following Lemma \ref{prop:ttowawa} and Lemma \ref{lemma:2019} with $t_0 = 0$.
...
\begin{lemma}
    ...
    $\int_{\Theta} V(\theta_0 \| \theta) \pi(\theta) \, \mathrm d \theta  < \infty$
where $V(\theta_0 \| \theta) = P_{\theta_0} \left( \log \left( p(X|\theta_0) / p(X | \theta) \right) - D_1 (\theta_0 \| \theta) \right)^2$.
Suppose as $n \to \infty$, $t \to 0$, $tn \to c \in (0,+\infty]$.
Then
\begin{align*}
     \int_{\Theta} \exp \{-t R_n (\theta_0, \theta)\} \pi (\theta) \, \mathrm d \theta  
     =
...
\begin{align*}
    ...
    (tn)^{-p/2 - 1}
     ...
     .
\end{align*}
The last inequality, combined with Lemma \ref{lemma:2019}, yields
\begin{align*}
    &
    P_{\theta_0}^n
    \left( 
