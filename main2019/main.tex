...
Bayes factor and its variants have been extensively studied in Bayesian hypotheses testing framework and have been shown to have good performance in a wealth of complex testing problems.
...
The good performance of Bayes factor in complex models
 motivates us to investigate the frequentist properties of the various Bayes factors and use them to construct frequentist tests.
...
We investigate the Wilks phenomenon of the Bayes factor and a class of variants of it named generalized fractional Bayes factor.
...
while the generalized fractional Bayes factor has Wilks phenomenon for general priors.
Frequentist tests based on Bayes factors are constructed using the Wilks phenomenon.
Extension to the general integrated likelihood ratio test is also considered.
For regular models, the proposed tests have the same asymptotic local power as the likelihood ratio test.
However, our theoretical results do not require the existence of the maximum likeihood estimate.
In particular, our methodology can be applied when the likelihood is unbounded.
Hence the proposed methodology has a wider application scope than the likelihood ratio test.
We use three examples to illustrate the behavior of the proposed methodology.
...
The universal applicability motivates us to use the Bayes factor as a test statistic and construct a frequentist test.
...
In fact, the required priors may be complicated for complex models.
...
Motivated by these methods,
we consider a class of statistics named generalized fractional Bayes factor which includes the posterior Bayes factor and fractional Bayes factor.
...
We prove that if the behavior of the weight functions is similar to the power posterior, then the general integrated likelihood ratio test also has Wilks phenomenon.
In particurlay, we show that the weight functions obtained by a variational method satisfies our requirements for the weight functions.
...
We show that the condiitions of our general theory is satisfied by the full-rank exponential family.
...
In fact, for the first submodel, the likelihood is unbounded and thus the LRT is not be defined.
...
Suppose $\theta=(\nu^\top ,\xi^\top )^\top $, where $\nu$ is a $p_0$ dimensional subvector and $\xi$ is a $p-p_0$ dimensional subvector and ``$\top$'' means transpose of a matrix.
...
In fact, Proposition \ref{prop:expansion} implies that the asymptotic null distribution of $\textrm{BF}_t (\BX^n)$ is free of the nuisance parameter $\nu$ if and only if
...
If the weight functions are power posterior densities, then Markov chain Monte Carlo (MCMC) methods can be used to sample from weight functions.
