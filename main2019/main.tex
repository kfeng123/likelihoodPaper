...
\begin{assumption}
    ...
    There exist $t^* \in (0,1)$ and $c^*, c^\dagger > 0$, $t_0^* \in (0,1)$ and $c_0^*, c_0^\dagger > 0$,
    \begin{align*}
...
\begin{proposition}\label{exponentialCon}
    ...
    $$I(\theta_0)=\frac{\partial^2}{\partial \theta \partial \theta^\top } A(\theta_0)>0.$$
    Then $L_{t}(\cdot;\BX^{n})$ is consistent for any $t>0$.
\end{proposition}
...
\begin{proof}[\textbf{Proof of Proposition~\ref{exponentialCon}}]
    ...
    \begin{split}
&
    \log \int_{\Theta} \exp\left\{ -t R_n (\theta_0 \| \theta) \right\} \pi(\theta) \, \mathrm d \theta
    \\
    \geq &
    \log \int_{\{\theta:\|\theta-\theta_0\|\leq M_n/\sqrt{n}\}}
\exp\left\{ -t R_n (\theta_0 \| \theta) \right\}
     \pi(\theta)\, d\theta
    \\
    =&
    \log\int_{\{h:h\leq M_n\}} \exp\left\{ t h^\top  I(\theta_0) \Delta_{n,\theta_0}-\frac{t}{2}h^\top  I(\theta_0) h\right\} \, dh
-\frac{p}{2}\log n+\log \pi(\theta_0)
    +o_{P_{\theta_0}^n}(1)
    \\
    =&
    \log\int_{\mathbb{R}^p} \exp\left\{ t h^\top  I(\theta_0)\Delta_{n,\theta_0}-\frac{t}{2}h^\top  I(\theta_0) h\right\} \, dh
-\frac{p}{2}\log n+\log \pi(\theta_0)
    +o_{P_{\theta_0}^n}(1)
    \\
    =&
    \frac{p}{2}\log\left(\frac{2\pi}{t n}\right)-\frac{1}{2}\log |I(\theta_0)|
        +\log \pi(\theta_0)
     +
     \frac{t}{2}\Delta_{n,\theta_0}^\top  I(\theta_0)\Delta_{n,\theta_0}
    +o_{P^n_{\theta_0}}(1).
