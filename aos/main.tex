...
\end{equation*}
where for $t>0$ and sets $\mathcal A\subset \Theta$, $\mathcal A_0\subset \tilde{\Theta}_0$,
\begin{align*}
...
For $t>0$, we say $L_t(\cdot;\BX^{(n)})$ is $\sqrt{n}$-consistent if for every $M_n\to \infty$,
    ...
    \begin{theorem}\label{Thm:maintheorem}
        ...
        $$
        where $\chi^2(p-p_0,\delta)$ is a noncentral chi-squared random variable with $p-p_0$ degrees of freedom and noncentrality parameter $\delta=\eta^T\big( I_{\theta_0}-I_{\theta_0} J(J^T I_{\theta_0} J)^{-1}J^T I_{\theta_0}\big)\eta$, $J=(\BI_{p_0},\mathbf{0}_{p_0\times(p-p_0)})^T$, $\BI_{p_0}$ is the $p_0$ dimensional identity matrix,
...
\begin{align*}
    ...
    \\
    =&\log \int_{\{h:\|h\|\leq M\}}\exp\left[ -\frac{t}{2}(h-\Delta_{n,\theta_0})^T I_{\theta_0}(h-\Delta_{n,\theta_0})\right] \, dh
    \\
    & +
...
Since $\Delta_{n,\theta_0}$ is bounded in probability, we have
        $$
            \begin{aligned}
            &\log \int_{\{h:\|h\|\leq M_n\}}\exp\left[ -\frac{t}{2}(h-\Delta_{n,\theta_0})^T I_{\theta_0}(h-\Delta_{n,\theta_0})\right] \, dh
                \\
                =&
                \log \int_{\mathbb{R}^p}\exp\left[ -\frac{t}{2}(h-\Delta_{n,\theta_0})^T I_{\theta_0}(h-\Delta_{n,\theta_0})\right] \, dh+o_{P^n_{\theta_0}}(1)
                \\
                =&
...
\end{aligned}
$$
It can be seen that $I_{\theta_0}^{(0)}= J^T I_{\theta_0}J$ and $\Delta_{n,\theta_0}^{(0)}=(J^T I_{\theta_0}J)^{-1} J^T I_{\theta_0} \Delta_{n,\theta_0}$.
Then
\begin{equation*}
    \begin{split}
            &\Delta_{n,\theta_0}^T I_{\theta_0} \Delta_{n,\theta_0}
...
&\log \frac{L_t(\Theta;\BX^{(n)})}{[ p_n(\BX^{(n)}|\theta_0)]^t}
    ...
    \\
    =&
    \log\int_{\{h:h\leq M_n\}} \exp\left[ t h^T I_{\theta_0}\Delta_{n,\theta_0}-\frac{t}{2}h^T I_{\theta_0}h\right] \, dh
    \\
&-\frac{p}{2}\log n+\log \pi(\theta_0)
    +o_{P_{\theta_0}^n}(1)
    \\
    =&
    \log\int_{\mathbb{R}^p} \exp\left[ t h^T I_{\theta_0}\Delta_{n,\theta_0}-\frac{t}{2}h^T I_{\theta_0}h\right] \, dh
    \\
&-\frac{p}{2}\log n+\log \pi(\theta_0)
    +o_{P_{\theta_0}^n}(1)
    \\
...
\begin{equation*}
    ...
    \end{pmatrix}.
\end{equation*}
Let $M>0$ be a fixed constant.  For $h=(h_1,h_2)^T\in \mathbb{R}^2$ such that $\|h\|\leq M$ and $i=1,\ldots, n$, we have
\begin{equation*}
    \begin{split}
...
\begin{equation*}
    ...
    |\sin \xi |\exp(-1/(2\tau)).
\end{equation*}
Hence if $(\xi, \tau)^T$ is close enough to $(0,1)^T$, then
\begin{equation*}
    \int \left|\sqrt{\tau}\phi(\sqrt{\tau}(X-\xi))-\phi(X)\right| d\mu
    \gtrsim |\xi|.
\end{equation*}
...
\begin{equation*}
    ...
    .
\end{equation*}
Hence if $(\xi, \tau)^T$ close enough to $(0,1)^T$, then
\begin{equation*}
    \begin{split}
        &\int \left|\sqrt{\tau}\phi(\sqrt{\tau}(X-\xi))-\phi(X)\right| d\mu
    \gtrsim |\tau-1|.
