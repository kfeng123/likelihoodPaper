...
Worse still, in some problems the likelihood is unbounded and hence the LRT is not defined; see, for example,~\cite{Cam1990Maximum}.
...
In fact, if the Bayes factor is treated as a frequentist test statistic,
 its asymptotic null distribution relies on the prior density evaluated at the true parameter value; see, for example,~\cite{clarke1990information}.
...
For many models, although the LRT fails, the likelihood function $L(\theta)$ still has good properties for most $\theta$ and  the MLE is unfortunately trapped in a fairly small area of $\theta$ where $L(\theta)$ has bad behavior. See Figure~\ref{myFigure1}.
...
In Section 2, we prove the Wilks phenomenon of the ILRT statistic and gives the asymptotic local power.
...
Because of the bad behavior of $L_t$ for $t>1$, next we only consider $L_t$ for $t\leq 1$.
 ...
 The consistency of posterior distribution has drawn much attention in the literature; see, e.g.,~\cite{ghosal2000},~\cite{Shen2001Rates},~\cite{vaart2007convergence}.
...
We shall prove the $\sqrt{n}$-consistency of $L_{t}$ for $0<t<1$ under certain conditions on the R\'{e}nyi divergence between distributions in the family $\{P_\theta:\theta\in\Theta\}$.
