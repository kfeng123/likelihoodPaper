...
Let $r>1$, on $A((\log n)^r)\cap \{\omega\geq n^{-s}\}$,
...
\end{align*}
Note that on $A((\log n)^r)\cap \{\omega\geq n^{-s}\}$, $\pi_{\xi}(\xi)=\pi_\xi(0)(1+o(1))$. Then
\begin{align*}
    ...
    {\color{red}\int_{-2\Phi^{-1}\big((\log n)^r/(2\omega \sqrt{n})+1/2\big)}^{2\Phi^{-1}\big((\log n)^r/(2\omega \sqrt{n})+1/2\big)} \exp\big\{
        t\omega \xi \sum_{i=1}^n X_i -\frac{1}{2} nt\omega^2 \xi^2
\big\} d\xi}
    \\
...
\frac{1}{\omega}
\bigg(
    \Phi\Big(2\sqrt{tn}\omega \Phi^{-1}\Big(\frac{(\log n)^r}{2\omega \sqrt{n}}+\frac{1}{2}\Big) -\sqrt{\frac{t}{n}}\sum_{i=1}^n X_i\Big)
    -
    \Phi\Big(-2\sqrt{tn}\omega \Phi^{-1}\Big(\frac{(\log n)^r}{2\omega \sqrt{n}}+\frac{1}{2}\Big)-\sqrt{\frac{t}{n}}\sum_{i=1}^n X_i\Big)
\bigg)
...
By Taylor expansion, we have $\Phi^{-1}(x+1/2)=\sqrt{2\pi} x+O(x^2)$ for $x\to 0$.
Hence uniformly for $(\log n)^{-1}\leq \omega\leq 1$, we have
\begin{equation*}
    \Phi^{-1}\Big(\frac{(\log n)^r}{2\omega \sqrt{n}}+\frac{1}{2}\Big)=\frac{\sqrt{2\pi}(\log n)^r}{2\omega \sqrt{n}}+O\Big(\frac{(\log n)^{2r}}{\omega^2 n}\Big).
\end{equation*}
Hence
\begin{equation*}
    2\sqrt{nt}\omega\Phi^{-1}\Big(\frac{(\log n)^r}{2\omega \sqrt{n}}+\frac{1}{2}\Big)=\sqrt{2\pi t}(\log n)^r+O\Big(\frac{(\log n)^{2r}}{\omega \sqrt{n}}\Big).
\end{equation*}
...
\begin{align*}
    & \int_{A( (\log n)^r )\cap \{\omega\leq (\log n)^{-1}\}} \Big[\prod_{i=1}^n \frac{p(X_i|\omega,\xi)}{p_0(X_i)}\Big]^t \pi(\omega,\xi)\, d\omega d\xi
    \\
    %=&\int_\Theta \exp\big\{
        %\omega \xi \sum_{i=1}^n Y_i -\frac{1}{2} \omega^2 n \big(\exp(\xi^2)-1\big)
    %\big\} \pi(\omega,\xi)\, d\omega d\xi\\
    =&\exp\big\{O_P(\log(\log n))\big\}
    \Pi\big(\omega(2\Phi(|\xi|/2)-1)\leq (\log n)^r n^{-1/2}, \omega\leq (\log n)^{-1}\big).
\end{align*}
We break the probability into two part: 
\begin{align*}
    &\Pi\big(\omega(2\Phi(|\xi|/2)-1)\leq (\log n)^r n^{-1/2}, \omega\leq (\log n)^{-1}\big)
    \\
    \leq &
    \Pi\big(\omega(2\Phi(|\xi|/2)-1)\leq (\log n)^r n^{-1/2}, \omega\leq  2(\log n)^r/\sqrt{n}\big)
    \\
    &
    +
    \Pi\big(\omega(2\Phi(|\xi|/2)-1)\leq (\log n)^r n^{-1/2},   2(\log n)^r/\sqrt{n} \leq \omega\leq (\log n)^{-1}\big)
    .
\end{align*}
...
\begin{align*}
    ...
    \\
    \leq &
    \Pi\big( \omega\leq  2(\log n)^r/\sqrt{n}\big)
    \lesssim 
    \int_{0}^{2(\log n)^r/\sqrt{n}} w^{\alpha_1-1}\, d\omega
...
\begin{align*}
    ...
    \\
    \leq
    &\Pi\big(|\xi|\leq C \omega^{-1} (\log n)^r n^{-1/2},   2(\log n)^r/\sqrt{n} \leq \omega\leq (\log n)^{-1}\big)
    \\
    \leq
    &\Pi\big(|\xi|\leq C \omega^{-1} (\log n)^r n^{-1/2},\omega\leq (\log n)^{-1}\big)
    \\
    \lesssim &
    \int_{0}^{(\log n)^{-1}} \omega^{\alpha_1-1}\, d\omega
...
\begin{align*}
    ...
    +
    \int_{{C(\log n)^r}/{(\epsilon \sqrt{n})}}^{(\log n)^{-1}} 
    2MC \omega^{\alpha_1-2}(\log n)^r n^{-1/2}
    \, d\omega
    \\
    \lesssim &
     \Big(\frac{(\log n)^r}{\sqrt{n}}\Big)^{\alpha_1}
     +\frac{(\log n)^r}{\sqrt{n}}\Bigg(\Big(\frac{(\log n)^r}{\sqrt{n}}\Big)^{\alpha_1-1}\vee \Big(\frac{1}{\log n}\Big)^{\alpha_1-1}\Bigg).
\end{align*}
