...
\frac{h_2}{4\sqrt{n}} \sum_{i=1}^n (1-X_i^2)
    ...
    \end{split}
\end{equation*}
where the $o_P(1)$ term is uniform for $\|h\|\leq M$.
...
\begin{equation*}
    ...
    =&
    \frac{1}{16}
    \left(\int \left|\sigma^{-1}\phi((X-\xi)/\sigma)-\phi(X)\right| d\mu\right)^2
    .
\end{split}
\end{equation*}
We have
\begin{equation*}
    \begin{split}
    &\int \left|\sigma^{-1}\phi((X-\xi)/\sigma)-\phi(X)\right| d\mu
    \geq
    \left|\int  exp(iX)\sigma^{-1}\phi((X-\xi)/\sigma)-exp(itX)\phi(X) d\mu\right|
    \\
    =&\left|\exp(i\xi -\sigma^2 /2)-\exp(-1/2)\right|.
\end{split}
\end{equation*}
On the one hand,
\begin{equation*}
...
\end{equation*}
Hence if $(\xi, \sigma^2)^T$ is close enough to $(0,1)$, we have
\begin{equation*}
    \int \left|\sigma^{-1}\phi((X-\xi)/\sigma)-\phi(X)\right| d\mu
    \gtrsim |\xi|.
\end{equation*}
On the other hand, if $(\xi, \sigma^2)^2$ is close enough to $(0,1)$
\begin{equation*}
    \begin{split}
    &\int \left|\sigma^{-1}\phi((X-\xi)/\sigma)-\phi(X)\right| d\mu
    \geq
    ...
    \\
    \geq &
    \left|\exp(-\sigma^2 /2)-\exp(-1/2)\right|\gtrsim |\sigma^2-1|.
\end{split}
\end{equation*}
Thus, there exist $\delta>0$ and $C>0$, such that for $\sqrt{\xi^2+(\sigma^2-1)^2}<\delta$, we have
\begin{equation*}
    D_{1/2}(\theta||\theta_0)\geq C (\xi^2+(\sigma^2-1)^2).
\end{equation*}
For $\sqrt{\xi^2+(\sigma^2-1)^2}\geq\delta$, we have
\begin{equation*}
    \begin{split}
    D_{1/2}(\theta||\theta_0)
    &\geq
    ...
    &\geq
    \frac{1}{16}
\left(\int \left(\sqrt{\sigma^{-1}\phi((X-\xi)/\sigma)}-\sqrt{\phi(X)}\right)^2 d\mu\right)^2
    .
\end{split}
\end{equation*}
...
Then by Theorem~\ref{Thm:maintheorem}, we have {\color{red} xxxxxxxx}.
\end{proof}
...
This implies that
\begin{equation*}
    2\log \Lambda_{a,b}=-\log(1+a/b)+\frac{a}{n}(\sum_{i=1}^n X_i)^2+o_P(1).
\end{equation*}
