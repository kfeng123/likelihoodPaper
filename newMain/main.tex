...
\begin{abstract}
    ...
    It is shown that the integrated likelihood ratio test shares similar frequency properties as the likelihood ratio test with fewer conditions.
    Our results hold even if the likelihood is unbounded where the likelihood ratio test can not be defined.
    ...
    We apply the proposed method to two testing problems in normal mixture model.
    %The normal mixture model is quite an irregular model.
    The likelihood ratio test can not be defined for the first problem and  has undesirable local power behavior for the second problem.
    In comparison, we prove that the integrated likelihood ratio test has good asymptotic power behavior in both problems.
...
For many models, although the LRT fails, the likelihood function $L(\theta)$ still has good properties for most $\theta$ and  the MLE is unfortunately trapped in a fairly small area of $\theta$ where $L(\theta)$ has bad behavior.
...
\begin{figure}
    ...
    \caption{
        ...
        We generate data $X_1,\ldots,X_n$ are iid from the mixture model $(1-\omega)\mathcal{N}(0,1)+\omega\mathcal{N}(\xi,\sigma^2)$ with $(\omega,\xi,\sigma^2)^T=(1/2,1,10^{-2})$ and $n=50$.
        We plot the likelihood function  versus $-\log (\sigma^2)$ with $\omega=1/2$ and $\xi=X_1$.
        The likelihood tends to infinity as $-\log (\sigma^2)$ tends to infinity.
        However, the behavior of the likelihood around the true parameter is fairly well.
    }
    \label{myFigure1}
\end{figure}
...
\end{equation*}
    \label{propositionTT}
\end{proposition}
This example shows that every when the LRT fails, the ILRT may still be valid and has the expected asymptotic distribution.
Thus, the ILRT methodology has a wider application scope than the LRT.


In the above example, we assume $\omega=1/2$ is known.
If $\omega$ in unknown, then the mixture model suffers from loss of identifiability and the behavior of the likelihood is fairly comlicated.
...
the likelihood has undesirable behavior due to loss of identifiability.
Although the LRT exists in this problem, its power behavior is not satisfactory.
In fact,~\cite{HALL2005158} showed that it has trivial power under $n^{-1/2}$ local alternative hypothesis. 
For this irregular problem, theorems in Section 2 can not be directly applied.
...
\end{enumerate}
    \label{mixtureThm}
\end{theorem}
Theorem~\ref{mixtureThm} shows that the ILRT has nontrival power if $\omega \xi $ is of order $n^{-1/2}$. 
...
We gave the asymptotic distribution of the generalized FBF, which is a special case of ILRT.
We also investigates the asymptotic behavior of the ILRT for general weight functions.
It is shown that the generalized FBF has a Wilks phenomenon similar to the LRT.
...
We also apply the ILRT methodology to two testing problems for the normal mixture model.
...
hence it can be expected that the ILRT method can have better properties than the LRT when the likilihood has complicated behavior.
The success of the ILRT methodology in our examples verifies this point.
It is interesting to apply the ILRT methodology to more problems where classical methods don't have good properties.
...
\end{equation*}
Hence if $(\xi, \tau)^T$ is close enough to $(0,1)$, we have
\begin{equation*}
    \int \left|\sqrt{\tau}\phi(\sqrt{\tau}(X-\xi))-\phi(X)\right| d\mu
    \gtrsim |\xi|.
\end{equation*}
On the other hand, for $(\xi, \tau)^T$ close enough to $(0,1)$, 
\begin{equation*}
    \begin{split}
        &\int \left|\sqrt{\tau}\phi(\sqrt{\tau}(X-\xi))-\phi(X)\right| d\mu
    \geq
...
\begin{proposition}\label{lastProp}
    ...
    \begin{equation*}
        P^n_{\theta_0} \int_{A(M_n)^c} \Big[\prod_{i=1}^n \frac{p(X_i|\omega,\xi)}{p_0(X_i)}\Big]^t \pi(\omega,\xi)\, d\omega d\mu =o(n^{-1/2}).
    \end{equation*}
\end{proposition}
\begin{proof}
\begin{equation*}
    P^n_{\theta_0} \int_{A(M_n)^c} \Big[\prod_{i=1}^n \frac{p(X_i|\omega,\xi)}{p_0(X_i)}\Big]^t \pi(\omega,\xi)\, d\omega d\xi
    =
...
\\
\leq & \exp \Big( -\frac{1}{4}(t\wedge (1-t)) \big(\int \big| p(X_i|\omega,\xi)-p_0(X_i)\big|\, d\mu \big)^2 \Big)
\\
...
This implies that
\begin{align*}
    &P^n_{\theta_0} \int_{A(M_n)^c} \Big[\prod_{i=1}^n \frac{p(X_i|\omega,\xi)}{p_0(X_i)}\Big]^t \pi_{\omega}(\omega)\pi_{\xi}(\xi)\, d\omega d\xi
\\
    \leq
    &
