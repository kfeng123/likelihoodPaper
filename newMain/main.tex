...
For two parameters $\theta_1$ and $\theta_2$, the R\'{e}nyi divergence of order $\alpha$ ($0<\alpha<1$) of $P_{\theta_1}$ from $P_{\theta_2}$ is defined to be
$$
...
\begin{theorem}
    ...
    Under Assumptions \ref{Assumption1} and~\ref{Assumption4}, for fixed $a\in(0,1)$ and any $M_n\to \infty$,
    $$
         \frac{L_{a} ( \{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\})}{L_{a}}
    \xrightarrow{P_{0}^n}  0
    .
    $$
\end{theorem}
\begin{proof}
    ...
    Note that
       ...
       \frac{L_{a} ( 
       \{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\})}
           {L_{a}(\Theta)}
=
    \frac{
        \int_{\big\{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\big\}} \Big[ \frac{p_n(\BX^{(n)}|\theta)}{p_n(\BX^{(n)}|\theta_0)} \Big]^{a} \pi(\theta) \, d \theta
    }{
        \int_{\Theta} \Big[ \frac{p_n(\BX^{(n)}|\theta)}{p_n(\BX^{(n)}|\theta_0)} \Big]^{a} \pi(\theta) \, d \theta
    }.
       \end{equation}
    ...
    $$
    \begin{aligned}
        &P_0^n\int_{\{\theta:\|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\} } \Big[ \frac{p_n(\BX^{(n)}|\theta)}{p_n(\BX^{(n)}|\theta_0)}  \Big]^{a} \pi(\theta) \, d \theta
        \\
        =&
        \int_{\{\theta:\|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\} } \left\{\int_{\mathcal{X}^n}\big[ {p_n} (\BX^{(n)}|\theta)\big]^a \big[ p_n (\BX^{(n)}|\theta_0) \big]^{1-a} \, d\mu^n \right\} \pi(\theta) \, d \theta\\
        =&
        \int_{\{\theta:\|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\} } \big[ \rho_{a}(\theta,\theta_0) \big]^n \pi(\theta) \, d \theta\\
        = &
        \int_{\{\theta:\|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\} } \exp \big[-(1-a)nD_a(\theta||\theta_0) \big] \pi(\theta) \, d \theta.
    \end{aligned}
    $$
    ...
    \begin{aligned}
        ...
        \exp \big[ -(1-a) {n} D_a(\theta,\theta_0) \big] \pi(\theta) \, d \theta
        \\
        =&\int_{\{\theta:\frac{M_n}{\sqrt{n}}\leq \|\theta-\theta_0\|\leq \delta \}}
        \exp\big[ -(1-a) {n} D_a(\theta,\theta_0) \big] \pi(\theta) \, d \theta+
        \int_{\{\theta: \|\theta-\theta_0\|>\delta\}} \exp\big[ -(1-a) {n} D_a(\theta,\theta_0) \big] \pi(\theta) \, d \theta
        \\
        \leq &
        \max_{\|\theta-\theta_0\|\leq \delta}\pi(\theta)
        \int_{\big\{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}} \big\}}
        \exp\big[ -(1-a)C {n} \|\theta-\theta_0\|^2 \big]
        \, d \theta
        +
        \exp\big[ -(1-a)\epsilon n\big].
    \end{aligned}
    $$
    Note that
    {\color{red}
    ...
    \begin{aligned}
        &\int_{\big\{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}} \big\}}
        \exp\big[-(1-a)C {n} \|\theta-\theta_0\|^2\big] \, d \theta
\\
        =&
...
\Big[
   ...
   \end{aligned}
    ...
    $$
     ...
     \begin{aligned}
         &
         P_0^n \left\{ \frac{L_{1/2} ( \{\theta: \|\theta-\theta_0\|\geq \frac{M_n}{\sqrt{n}}\})}{L_{1/2}(\Theta)}
         \right\}
         \leq
