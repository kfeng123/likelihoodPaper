...
In this section, we consider binomial mixture model.

Suppose $X_1, \dots, X_n$ are iid distributed as a mixture of binomial distributions
\begin{align*}
    p(x | \omega, p_1, p_2 ) 
    =
    (1- \omega) {k \choose x} (1- p_1)^{k-x} p_1^{x}
    + \omega {k \choose x} (1- p_2)^{k-x} p_2^{x},
\end{align*}
where  $0< \omega, p_1, p_2 < 1$ are unknown parameters and $k \geq 3$ is known.
...
To facilitate the derivation, we reparameterize the model as $u = p_1, \xi= p_2 - p_1$.
Then $\theta = (\omega, u, \xi)^\top$, $\Theta = \{(\omega, u, \xi)^\top :   \xi \in (-u, 1-u), u\in (0,1) , \omega \in (0, 1) \}$, and
\begin{align*}
    p(x| \omega, u, \xi) = 
{k \choose y} (1- u)^{k-x} u^{x}
\left[
1 + \omega 
\left(
    {\left(1-\frac{\xi}{1-u}\right)^{k-x} \left(1+ \frac \xi u \right)^x}
-1
\right)
\right].
\end{align*}
The hypotheses of interest becomes
\begin{align*}
    H: \xi  = 0
    \quad
    \text{vs.}
...
Let $\theta_0 = (\omega_0, u_0, 0)^\top$.

Let $M > 0$ be a fixed constant.
Let $h = (h_1, h_2, h_3)^\top \in \mathbb R^3$ such that $\|h\| \leq M$.
Then for sufficiently large $n$, $\theta_0 + n^{-1/2}h \in \Theta$.
We have
\begin{align*}
    \log
    \frac
    ...
    (k-x)
    \log\left(
        1-\frac{n^{-1/2} h_2}{1-u_0}
    \right)
    +
    x
    \log
    \left(
        1+\frac{n^{-1/2} h_2}{u_0}
    \right)
    \\
    &
    +
    \log\left[
        ...
        \left(
            \left(
                1- \frac{n^{-1/2} h_3}{1-u_0 - n^{-1/2} h_2}
            \right)^{k-x}
            \left(
                1+ \frac{n^{-1/2} h_3}{u_0 + n^{-1/2} h_2}
            \right)^{x}
            -1
        \right)
    \right].
\end{align*}
Note that as $t \to 0$, uniformly for $j=0 ,\dots, k$,
\begin{align*}
    (1+t)^j = 1 + jt + \frac{j(j-1)}{2} t^2 + O(t^3),
    \quad
    \log(1+t) = t - \frac{1}{2}t^2 + O(t^3),
    \quad
    \frac{1}{1-t} = 1 + t + t^2 + O(t^3)
    .
\end{align*}
Then uniformly for $x = 0, \dots, k$,
\begin{align*}
    (k-x)
    \log\left(
        1-\frac{n^{-1/2} h_2}{1-u_0}
    \right)
    =-\frac{(k-x) h_2}{1-u_0}n^{-1/2} - \frac{(k-x) h_2^2}{2(1-u_0)^2}n^{-1}
    +O(n^{-3/2}),
\end{align*}
\begin{align*}
    x
    \log\left(
        1+\frac{n^{-1/2} h_2}{u_0}
    \right)
    =\frac{x h_2}{u_0}n^{-1/2} - \frac{x h_2^2}{2 u_0^2} n^{-1}
    +O(n^{-3/2}).
\end{align*}
\begin{align*}
    \frac{n^{-1/2}h_3}{1-u_0 - n^{-1/2} h_2 }
    =\frac{h_3}{1-u_0} n^{-1/2}
    +\frac{h_2 h_3}{(1-u_0)^2} n^{-1} +O(n^{-3/2}).
\end{align*}
\begin{align*}
    \left(
        1-\frac{n^{-1/2}h_3}{1-u_0 - n^{-1/2} h_2 }
    ...
    =&
    1-
    \frac{(k-x) h_3}{1-u_0} n^{-1/2}
    -\frac{(k-x) h_2 h_3}{(1-u_0)^2} n^{-1} 
    +\frac{(k-x) (k-x-1) h_3^2}{ 2 (1-u_0)^2} n^{-1}
    +O(n^{-3/2})
    \\
    =&
    1-
    \frac{(k-x) h_3}{1-u_0} n^{-1/2}
    +\left( 
    \frac{(k-x) (k-x-1) h_3^2}{ 2 (1-u_0)^2}
        - \frac{(k-x) h_2 h_3}{(1-u_0)^2}  
\right)n^{-1}
    +O(n^{-3/2})
    .
\end{align*}
\begin{align*}
    \frac{n^{-1/2}h_3}{u_0 + n^{-1/2} h_2}
=
\frac{h_3}{u_0} n^{-1/2} - \frac{h_2 h_3}{u_0^2} n^{-1} + O(n^{-3/2}).
\end{align*}
\begin{align*}
    \left(
        1+\frac{n^{-1/2}h_3}{u_0 + n^{-1/2} h_2}
...
=&
1+
\frac{x h_3}{u_0} n^{-1/2} - \frac{x h_2 h_3}{u_0^2} n^{-1} 
+\frac{x(x-1)h_3^2}{2 u_0^2} n^{-1}
+O(n^{-3/2})
    \\
=&
1+
\frac{x h_3}{u_0} n^{-1/2}
+
\left(
\frac{x(x-1)h_3^2}{2 u_0^2}
- \frac{x h_2 h_3}{u_0^2} 
\right)
n^{-1}
+O(n^{-3/2}).
\end{align*}
...
1
+
\frac{x- u_0 k}{u_0 (1- u_0)}
    h_3 n^{-1/2}
    \\
    &+
\left(
    \frac{
(x- u_0 k)^2 +(2u_0 - 1)x - k u_0^2  
}{2u_0^2 (1-u_0)^2}
h_3^2
+
    \frac{(2u_0 - 1)x - k u_0^2 }{u_0^2 (1-u_0)^2}
h_2 h_3
\right)
n^{-1}
    +O(n^{-3/2})
...
\left(
        \omega_0
        \frac{(1-\omega_0) (x- u_0 k)^2 +(2u_0 - 1)x - k u_0^2  }{2u_0^2 (1-u_0)^2}
h_3^2
+
        \omega_0
    \frac{(2u_0 - 1)x - k u_0^2 }{u_0^2 (1-u_0)^2}
...
\begin{align*}
    ...
    \\
    =&
    \frac{x- u_0 k}{u_0 (1- u_0)} (h_2 + \omega_0 h_3) n^{-1/2}
    \\
    &+
\Bigg(
    \frac{(2u_0 -1) x - k u_0^2  }{2u_0^2 (1-u_0)^2}
    h_2^2
    +
        \omega_0
        \frac{(1-\omega_0) (x- u_0 k)^2 +(2u_0 - 1)x - k u_0^2  }{2u_0^2 (1-u_0)^2}
...
+O(n^{-3/2}),
\end{align*}
where the $O(\cdot)$ term is uniform for $x = 0, \dots, k$.

Thus,
\begin{align*}
    R_n(\theta_0 \| \theta_0 + n^{-1/2}h) 
    =&
    -\frac{\bar \BX- u_0 k }{u_0 (1- u_0)} (h_2 + \omega_0 h_3) n^{1/2}
    \\
    &-
\Bigg(
    \frac{(2u_0 -1) \bar \BX - k u_0^2  }{2u_0^2 (1-u_0)^2}
    h_2^2
    +
        \omega_0
        \frac{(1-\omega_0)  \frac{1}{n}\sum_{i=1}^n (\BX_i- u_0 k )^2 +(2u_0 - 1)\bar \BX - k u_0^2  }{2u_0^2 (1-u_0)^2}
h_3^2
\\
&
+
        \omega_0
    \frac{(2u_0 - 1) \bar \BX - k u_0^2 }{u_0^2 (1-u_0)^2}
h_2 h_3
+
\frac{\bar \BX- u_0 k }{u_0 (1- u_0)}
    h_1 h_3 
\Bigg)
+O(n^{-1/2})
\\
    =&
    -\frac{ h_2 + \omega_0 h_3 }{u_0 (1- u_0)}  n^{1/2}  (\bar \BX- u_0 k)
    \\
    &+
    \frac{k
(
...
\begin{proposition}
    Let 
    \begin{align*}
    A(M_n)=\{(\omega, p_1, p_2): \max(
&\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
,
\left(  p_2 - p_0 + (1-\omega) (p_1 - p_2)  \right)^2
,
\\
&
\omega^2 (p_2 - p_0)^2 (p_2 - p_1)^2
,
(1-\omega)^2 (p_1 - p_0)^2 (p_2 - p_1)^2
    )\leq M_n n^{-1}\}.
    \end{align*}
    Let $0<t<1$ be a constant.
    If $M_n \geq \log n$, then
    \begin{equation*}
        P^n_{\theta_0} \int_{A(M_n)^\complement}  \left\{\prod_{i=1}^n \frac{p(X_i|\omega, p_1, p_2 )}{p(X_i| 0, p_0, 0)}\right\}^t \pi(\omega,p_1, p_2)\, \mathrm  d\omega \mathrm d p_1 \mathrm d p_2 =
    \end{equation*}
\end{proposition}
\begin{proof}
    It can be seen that 
\begin{equation*}
    ...
    \\
    =&
    \int_{A(M_n)^\complement} \left( \int p( x |\omega, p_1, p_2)^t p( x |0, p_0, 0)^{1-t}\, \mathrm d \mu (x) \right)^n \pi(\omega, p_1, p_2)\, \mathrm d\omega \mathrm d p_1 \mathrm d p_2.
    \end{split}
\end{equation*}
Note that
\begin{align*}
    \sum_{x=0}^k p( x |\omega, p_1, p_2)^t p( x |0, p_0, 0)^{1-t}
    \leq&  \left( \sum_{x=0}^k \sqrt{p( x |\omega, p_1, p_2) p( x |0, p_0, 0)} \right)^{2(t\wedge (1-t))}
    \\
    = & \left\{1-\frac{1}{2} \sum_{x=0}^k \left(\sqrt{p( x |\omega, p_1, p_2) }-\sqrt{p( x |0, p_0,0)}\right)^2 \right\}^{2(t\wedge (1-t))}
\\
\leq & \exp \left\{ -(t\wedge (1-t)) \sum_{x=0}^k \left(\sqrt{p( x |\omega,p_1, p_2) }-\sqrt{p( x |0, p_0 ,0)}\right)^2 \right\}
.
\end{align*}
We have
\begin{align*}
\sum_{x=0}^k \left(\sqrt{p( x |\omega,p_1, p_2) }-\sqrt{p( x |0, p_0 ,0)}\right)^2 
=&
\sum_{x=0}^k \left(\frac{{p( x |\omega,p_1, p_2) }-{p( x |0, p_0 ,0)}}{\sqrt{p( x |\omega,p_1, p_2) }+\sqrt{p( x |0, p_0 ,0)}}\right)^2 
\\
\geq&
\frac{1}{4}
\sum_{x=0}^k \left({p( x |\omega,p_1, p_2) }-{p( x |0, p_0 ,0)}\right)^2 
\\
\geq&
\frac{1}{4}
\sum_{x=0}^k
\left((1-\omega) (1-p_1)^{k-x} p_1^{x} + \omega (1-p_2)^{k-x} p_2^x - (1-p_0)^{k-x} p_0^x \right)^2 
\\
=&
\frac{1}{4} \| \BA \gamma \|^2
.
...
Note that for $x > 0$,
\begin{align*}
    &(1-\omega) (1-p_1)^{k-x} p_1^{x} + \omega (1-p_2)^{k-x} p_2^x - (1-p_0)^{k-x} p_0^x 
    \\
    =&
    \begin{pmatrix}
        0&  \cdots & 0 & {k-x \choose 0} & {k-x \choose 1} (-1) & \cdots & {k-x \choose k-x } (-1)^{k-x}
    \end{pmatrix}
    \begin{pmatrix}
        (1-\omega) p_1 + \omega p_2 - p_0 \\
        (1-\omega) p_1^{2} + \omega p_2^{2} - p_0^{2} \\
        \vdots
        \\
        (1-\omega) p_1^k + \omega p_2^k - p_0^k \\
    \end{pmatrix}.
\end{align*}
And 
\begin{align*}
    &(1-\omega) (1-p_1)^{k}  + \omega (1-p_2)^{k}  - (1-p_0)^{k} 
    \\
    =&
    \begin{pmatrix}
        {k \choose 1} (-1) & \cdots & {k \choose k } (-1)^{k}
    \end{pmatrix}
    \begin{pmatrix}
        (1-\omega) p_1 + \omega p_2 - p_0 \\
        (1-\omega) p_1^{2} + \omega p_2^{2} - p_0^{2} \\
...
\begin{align*}
    \begin{pmatrix}
 &(1-\omega) (1-p_1)^{k} p_1^{x} + \omega (1-p_2)^{k} p_2^x - (1-p_0)^{k} p_0^x     
 \\
 &\vdots
 \\
 &(1-\omega) (1-p_1)^{k-x} p_1^{x} + \omega (1-p_2)^{k-x} p_2^x - (1-p_0)^{k-x} p_0^x     
    ...
    \end{pmatrix},
\end{align*}
where $\BA$ is a $(k+1) \times k$ matrix with $(\BA)_{i,j} ={ k -i + 1 \choose j - i + 1 }(-1)^{j-i + 1} $ for $j \geq i - 1$ and $(\BA)_{i,j} = 0$ otherwise.
Then
\begin{align*}
    &
    \sum_{x=0}^k \frac{{k \choose x}}{(1-p_0)^{k-x} p_0^x}
    \left(
        (1-\omega ) (1-p_1)^{k-x} p_1^x + \omega (1-p_2)^{k-x} p_2^x - (1-p_0)^{k-x} p_0^x
    \right)^2
    \\
...
Note that $\BA$ has rank $k$, hence for $k\geq 2$,
\begin{align*}
\sum_{x=0}^k \left(\sqrt{p( x |\omega,p_1, p_2) }-\sqrt{p( x |0, p_0 ,0)}\right)^2  
\gtrsim
&
...
\geq
&
 \left( (1-\omega) p_1 + \omega p_2 - p_0 \right)^2
 +\left( (1-\omega) p_1^2 + \omega p_2^2 - p_0^2 \right)^2.
\end{align*}
Note that
\begin{align*}
 &\left( (1-\omega) p_1 + \omega p_2 - p_0 \right)^2
...
=&
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+\left(  p_1^2 - p_0^2  + \omega (p_2^2 - p_1^2) \right)^2
 \\
=&
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+
(p_1 + p_0)^2
\left(  p_1 - p_0  + \omega \frac{p_2^2 - p_1^2}{p_1 + p_0} \right)^2
 \\
=&
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+
(p_1 + p_0)^2
\left(  p_1 - p_0 + \omega (p_2 - p_1) + \omega \frac{(p_2 - p_0)(p_2 - p_1)}{p_1 + p_0} \right)^2
 \\
\gtrsim&
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+
...
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+
\left(   \omega \frac{(p_2 - p_0)(p_2 - p_1)}{p_1 + p_0} \right)^2
 \\
\gtrsim&
\left(  p_1 - p_0 + \omega (p_2 - p_1)  \right)^2
+
...
=&
\left(  p_2 - p_0 + (1-\omega) (p_1 - p_2)  \right)^2
+\left(  p_2^2 - p_0^2  + (1-\omega) (p_1^2 - p_2^2) \right)^2
 \\
\gtrsim&
\left(  p_2 - p_0 + (1-\omega) (p_1 - p_2)  \right)^2
+
...
(1-\omega)^2 (p_1 - p_0)^2 (p_2 - p_1)^2
    ...
    Then
    \begin{equation*}
        \int_{A(\log n )} \left\{\prod_{i=1}^n \frac{p(X_i|\omega, p_1, p_2 )}{p(X_i| 0, p_0, 0)}\right\}^t \pi(\omega,p_1, p_2)\, \mathrm  d\omega \mathrm d p_1 \mathrm d p_2 =
         .
    \end{equation*}
\end{proposition}
\begin{proof}
...
\begin{align*}
    ...
    =
    (1- \omega)  \frac{(1- p_1)^{k-x} p_1^{x}}{(1- p_0)^{k-x} p_0^{x}}
    + \omega  \frac{(1- p_2)^{k-x} p_2^{x}}{(1- p_0)^{k-x} p_0^{x}}.
\end{align*}
Note that as $t \to 0$, uniformly for $j=0 ,\dots, k$,
\begin{align*}
    (1+t)^j = 1 + jt + \frac{j(j-1)}{2} t^2 + O(t^3),
...
\begin{align*}
    ...
    \\
    =&
    (1-p_0 -( p_1 - p_0))^{k-x} (p_0 + p_1 - p_0)^x -
    (1-p_0)^{k-x} p_0^x .
\end{align*}
Note that
\begin{align*}
...
\\
= &
    (1-p_0)^{k-x} -(k-x)(1-p_0)^{k-x-1}( p_1 - p_0) +\frac{(k-x)(k-x-1)}{2} (1-p_0)^{k-x-2}(p_1 - p_0)^2 
    + O(|p_1 - p_0|^3).
\end{align*}
And
\begin{align*}
(p_0 + p_1 - p_0)^x
=
p_0^x + x p_0^{x-1} (p_1 - p_0) + \frac{x(x-1)}{2} p_0^{x-2}(p_1 - p_0)^2  
+ O(|p_1 - p_0|^3).
\end{align*}
Hence
\begin{align*}
...
=&
    (1-p_0)^{k-x}p_0^x
    + \left(\frac{x}{p_0} -  \frac{k-x}{1-p_0} \right)(1-p_0)^{k-x} p_0^x ( p_1 - p_0) 
    \\
    &
    + \left(\frac{x(x-1)}{2 p_0^2 } + \frac{(k-x)(k-x-1)}{2(1-p_0)^2} - \frac{x(k-x)}{p_0 (1- p_0 )} \right)(1-p_0)^{k-x} p_0^x ( p_1 - p_0)^2
    + O(|p_1 - p_0|^3).
\end{align*}
Thus, uniformly for $x = 0, 1,\dots , k$ and $p_1 \in (0,1)$,
\begin{align*}
    &\frac{(1-p_1)^{k-x} p_1^x}
    {(1-p_0)^{k-x} p_0^x }
    \\
    =&
     1 + 
    \left(\frac{x}{p_0} -  \frac{k-x}{1-p_0} \right) ( p_1 - p_0) 
    \\
    &
    + \left(\frac{x(x-1)}{2 p_0^2 } + \frac{(k-x)(k-x-1)}{2(1-p_0)^2} - \frac{x(k-x)}{p_0 (1- p_0 )} \right) ( p_1 - p_0)^2
    + O(|p_1 - p_0|^3).
\end{align*}
Thus, uniformly for $x = 0, 1,\dots , k$ and $p_1, p_2 \in (0,1)$,
\begin{align*}
    & 
    \frac{p(x|\omega, p_1 , p_2)}{p(x|0, p_0, 0)}
    \\
    =&
     1 + 
     \left(\frac{x}{p_0} -  \frac{k-x}{1-p_0} \right) \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
    \\
    &
    + \left(\frac{x(x-1)}{2 p_0^2 } + \frac{(k-x)(k-x-1)}{2(1-p_0)^2} - \frac{x(k-x)}{p_0 (1- p_0 )} \right) \left((1-\omega)( p_1 - p_0)^2 + \omega (p_2 - p_0)^2 \right)
    \\
    &
    + O((1-\omega)|p_1 - p_0|^3 + \omega |p_2 - p_0|^3)
    \\
    =&
     1 + 
     \left(\frac{x}{p_0} -  \frac{k-x}{1-p_0} \right) \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
...
+ \frac{1}{2}
\left(
    \left(\frac{x}{ p_0 } - \frac{k-x}{1- p_0 } \right)^2 
    -\frac{x}{p_0^2} - \frac{k-x}{(1-p_0)^2}
\right)
\left((1-\omega)( p_1 - p_0)^2 + \omega (p_2 - p_0)^2 \right)
    \\
    &
    + O((1-\omega)|p_1 - p_0|^3 + \omega |p_2 - p_0|^3).
\end{align*}

We have, uniformly for $x = 0, 1,\dots , k$ and $p_1, p_2 \in (0,1)$,
\begin{align*}
    & 
    \log \frac{p(x|\omega, p_1 , p_2)}{p(x|0, p_0, 0)}
    \\
    =&
     \left(\frac{x}{p_0} -  \frac{k-x}{1-p_0} \right) \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
    \\
...
\begin{align*}
    & 
    \sum_{i=1}^n \log \frac{p( X_i |\omega, p_1 , p_2)}{p( X_i |0, p_0, 0)}
    \\
    =&
    \sum_{i=1}^n \left(\frac{X_i}{p_0} -  \frac{k-X_i}{1-p_0} \right) \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
    \\
    &
+ \frac{1}{2}
\sum_{i=1}^n \left(
    \left(\frac{X_i}{ p_0 } - \frac{k-X_i}{1- p_0 } \right)^2 
    -\frac{X_i}{p_0^2} - \frac{k-X_i}{(1-p_0)^2}
\right)
\left((1-\omega)( p_1 - p_0)^2 + \omega (p_2 - p_0)^2 \right)
    \\
    &
    ...
    \sum_{i=1}^n
    \Bigg(
     \left(\frac{X_i}{p_0} -  \frac{k-X_i}{1-p_0} \right) \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
\Bigg)^2
    \\
    &
    + O((1-\omega) n |p_1 - p_0|^3 + \omega  n |p_2 - p_0|^3).
\end{align*}

Note that
\begin{align*}
    \myE \left(\frac{X_1}{p_0} -  \frac{k-X_1}{1-p_0} \right) = 0,
    \quad
    \myVar \left(\frac{X_1}{p_0} -  \frac{k-X_1}{1-p_0} \right) = \frac{k}{p_0 (1-p_0)}.
\end{align*}
Thus,
\begin{align*}
    & 
    ...
    -\frac{1}{2}
    \sum_{i=1}^n
     \left(\frac{X_i}{p_0} -  \frac{k-X_i}{1-p_0} \right)^2 \left((1-\omega) p_1 + \omega p_2 - p_0 \right)^2
    \\
    &
    +
    O_p\left( (1-\omega)\sqrt{n} ( p_1 - p_0)^2 + \omega \sqrt{n} (p_2 - p_0)^2 \right)
    + O((1-\omega) n |p_1 - p_0|^3 + \omega  n |p_2 - p_0|^3)
    \\
    =&
    \frac{1}{\sqrt n}\sum_{i=1}^n \left(\frac{X_i}{p_0} -  \frac{k-X_i}{1-p_0} \right) \sqrt{n} \left((1-\omega) p_1 + \omega p_2 - p_0 \right)
    - \frac{k }{2 p_0 (1-p_0)}
    n
     \left((1-\omega) p_1 + \omega p_2 - p_0 \right)^2
    \\
...
{\color{red} there seems an error here}
...
\\
    ...
    \leq
    &
    \int_{A(M_n)^\complement} \exp \left\{ -(t\wedge (1-t)) M_n^2 \right\}
    \pi_{\omega}(\omega) \pi_\xi(\xi)\, \mathrm d\omega \mathrm d\xi
\\
    \leq
    &
    n^{-1/2}
    \int_{A(M_n)^\complement} 
\pi_{\omega}(\omega) \pi_\xi(\xi)\, \mathrm d\omega \mathrm d\xi
\\
=&{\color{red}o(n^{-1/2})}.
\end{align*}
{\color{red} Seems should be O() instead of o().}
This completes the proof.
...
On $A((\log n)^r)\cap \{\omega\geq n^{-s}\}$, we have $\max_{1\leq i\leq n}|\xi X_i-\xi^2/2|\leq |\xi| \max_{1\leq i\leq n}|X_i|+\xi^2/2=O_{P_{\theta_0}^n} (|\xi|(\log n)^{1/2})$.
...
\begin{align*}
    ...
    =&\exp\left\{O_{P^n_{\theta_0}}(\log\log n)\right\}
