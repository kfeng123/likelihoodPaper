...
%Bayes factor is sensitive to the specification of prior, which may cause difficulties in the absence of a well-formulated subjective prior; see, e.g., \cite{Lindley1982}.
...
%Many work has been done on specific testing problems when the LRT does not work well.
%Many work has been done on the testing problems when the LRT does not work well; see, e.g., \cite{chen2009}.
%However, most existing work only concerns specefic models.
...
%k, and can not be directly used to construct significance test.
%A main reason is that the Bayes factor methodology does not intend to control the frequentist type I error rate and hence does not form a significance test.
%, and hence can not be directly treated as a significance test.
%In fact, it is known that Bayes factor is sensitive to the choice of prior distribution. 
%In fact, as shown by~\cite{clarke1990information},
%the asymptotic distribution of Bayes factor depends on the prior density at the true parameter.
%the Bayes factor for point null hypothesis, and applied the result to construct significance tests.
%\cite{Johnson2013a}.
%posterior predictive p value.
%\cite{clarke1990information} investigated the asymptotic distribution of the Bayes factor and constructed significance test for point null hypothesis.
%On the other hand, there is relatively few work which Bayes factor from a frequentist viewpoint.
%Note that the historical development of Bayes factor is almost orthogonal to significance test.
%However, this idea is not fully developed.
...
%In fact, the Bayes factor methodology rejects the null hypothesis when the Bayes factor exceeds certian threshold.
 ...
 %and hence does not produce valid significance test in general.
...
%It is known that the MLE does not exist.
...
%Although the idea has a long history, most existing work only considered specific models.
%To the best of our knowledge, there is no mathematically rigorous work on the asymptotic distribution of the Bayes factor and its variants.
...
Suppose that the possible distribution $P_\theta$ of $X_i$ has a density $p(x|\theta)$ with respect to $\mu$, a $\sigma$-finite measure on $\mathscr{A}$. %$\mathcal{X}$.
...
%it is a common practice to use improper prior for certain parameters, in this case, $\int_{\Theta} \pi(\theta) \, \mathrm d \theta = + \infty$ or $\int_{\tilde \Theta_0} \pi_0 (\nu) \, \mathrm d \nu  = + \infty$.
...
%There's an open neighborhood $V$ of $\theta_0$ such that for every $\theta_1,\theta_2\in V$,
        ...
        %The Fisher information matrix $I_{\theta_0}=P_{\theta_0}\dot{\ell}_{\theta_0}\dot{\ell}_{\theta_0}^\myT $ is positive-definite and as $\theta\to \theta_0$,
    %\begin{equation*}
        %P_{\theta_0} \log \frac{p(X|\theta)}{ p(X|\theta_0)}
        %=-\frac{1}{2}(\theta-\theta_0)^\myT  I_{\theta_0} (\theta-\theta_0)+o(\|\theta-\theta_0\|^2).
    %\end{equation*}
...
For every $M>0$,
    ...
    \end{align*}
    %\begin{align*}
    %R_n (\theta\| \theta' )= \log \left\{ p_n(\BX^{n}|\theta)/ p_n(\BX^{n}|\theta') \right\}
    %\end{align*}
...
%In this case, priors satisfying \eqref{eq:shouldConst2} may be hard to construct.
...
%Now we turn to the variants of the Bayes factor.
...
%To investigate the other two variants of the Bayes factor,
...
%\rho_{t}(\theta_1,\theta_2)
,
$$
%where
%$
%\rho_{t}(\theta_1,\theta_2)=\int_{\mathcal{X}} p(x|\theta_1)^{t} p(x|\theta_2)^{1-t} \, \mathrm d \mu (x)
...
\end{assumption}
    ...
    %The condition
        %$\int_{\Theta} \exp\left\{ -c^* D_{1-t^*}(\theta_0 \| \theta) \right\} \pi (\theta) \, \mathrm d \theta < \infty$
        %is obviously satisfied for proper priors.
...
%, and if the prior is not proper, which is often the case for Bayes factor, existing results can not be directly applied.
...
%Suppose $X_1, \dots, X_n \in \mathbb R^p$ are non-random predictors.
...
Suppose $y_1, \dots, y_n$ are independent observations taking values on $\{-1,1\}$ and have density function
    ...
    %This phenomenon rise to additional difficulty numerical optimization.
    %If the MLE does not exist, the LRT is and may have undesirable behavior.
...
%which may not exist when $\xi$ is largely deviated from $\xi_0$.
%The fact that the MLE may not exist may lead to the abnormal behavior of the Wald test.
%However, if the MLE.
%The commonly used test procedure for this problem is the LRT.
%, it also has some weaknesses.
%Although the LRT is commonly used for this problem, it also has some weaknesses.
%For example, if the data points are completely separated under the null hypothesis, then \cite{Albert1984On}, Theorem 1 implies that the LRT is equal to $1$ and the null hypothesis can not be rejected.
...
%It is noted by \cite{Candes2020The_phase} that in the present setting, quasi-complete separation occurs with probability $0$.
%On the other hand, if completely separation occurs, the maximum likelihood is equal to $1$ although the MLE does not exist; see \cite{Albert1984On}, Theorem 1.
%Hence the LRT is indeed well defined in the present setting.
...
%Note that the generalized FBF does not require an estimator of the parameter and is always well defined provided the the prior is proper.
...
%Our simulations also show that the generalized FBF and the LRT outperform
%that when the nuisance parameter $\nu$ is deviated from $0$, the LRT suffers from severe level inflation, while the generalized FBF has relatively reasonable empirical size.

%If the MLE does not exist, the LRT can still be defined since the numerator and the denominator of \eqref{logistic:lrt} are both bounded by $1$.
%\begin{align}\label{logistic:lrt}
%    \Lambda_{\text{LRT}} (y_1, \dots, y_n) = 
%    \frac{
%        \sup_{\theta \in \mathbb R^p} \prod_{i=1}^n p (y_i | \theta)
%    }{
%        \sup_{\nu \in \mathbb R^{p_0}} \prod_{i=1}^n p (y_i | \nu, \xi_0)
%    }.
%\end{align}
%It is known that the MLE for the logistic regression model does not exist when the data points are completely separated or quasi-completely separated; see, e.g., \cite{Albert1984On,Candes2020The_phase}.
%If the MLE does not exist, the LRT can still be defined since the numerator and the denominator of \eqref{logistic:lrt} are both bounded by $1$.
%In this case, however, the property of the LRT may not be desirable.
%In fact, if the data points are completely separated under the null hypothesis, then \cite{Albert1984On}, Theorem 1 implies $
%\Lambda_{\text{LRT}} (y_1, \dots, y_n) = 1$.

    %We consider the model conditioning on $X_1,\dots,X_n$, that is, $X_1, \dots, X_n$ are assumed to be non-random.
...
Since the integral of the likelihood can smooth the irregular behavior of the likelihood, it can be expected that Bayes methods may have better behavior than the LRT.
...
%\subsection{Three-parameter lognormal model}\label{sec:lognormal}
%
%Threshold parameter $\gamma$.
%
%If $\gamma$ is known, then the model reduces to the two-parameter lognormal model, and the statistical inference methods are well understood.
%See, e.g., \cite{Lawless2003Statistical}.
%
%Suppose $X_1, \dots, X_n$ are iid random variables from a three-parameter lognormal distribution
%\begin{align*}
%p(x | \gamma, \mu, \sigma^2) = 
%\frac{1}{(x-\gamma)\sqrt{2\pi \sigma^2 }}
%\exp\left(
%    - \frac{(\log(x-\gamma)-\mu)^2}{2\sigma^2}
%\right)
%\mathrm 1_{(x > \gamma)}.
%\end{align*}
%The parameter space is
%$\Theta = \{\theta = (\gamma, \mu, \sigma)^\myT | \gamma, \mu \in \mathbb R, \, \sigma \in \mathbb R^+ \} $.
%
%Then the likelihood function is
%\begin{align*}
%p_n(\BX^n | \gamma, \mu, \sigma^2) = 
%\frac{1}{\prod_{i=1}^n (X_i-\gamma) {(2\pi \sigma^2)^{n/2}}}
%\exp\left(
%    - \frac{\sum_{i=1}^n (\log(X_i-\gamma)-\mu)^2}{2\sigma^2}
%\right)
%\mathrm 1_{(X_{(1)} > \gamma)},
%\end{align*}
%where $X_{(1)} \leq \dots \leq X_{(n)}$ are the order statistics of $X_1, \dots, X_n$.
%
%We consider the noninformative prior distribution $\pi(\gamma, \mu, \sigma^2) = \frac{1}{\sigma^2}$.
%
%By direct calculation, we have
%%For $\gamma < X_{(1)}$, we have
%\begin{align*}
%    &
...
%\pi(\gamma, \mu, \sigma^2)
%\,
%\mathrm d \mu \mathrm d \sigma^2
%\\
%= &
%\frac{1}{ \sqrt { n}{\pi^{(n-1)/2}} \prod_{i=1}^n (X_i-\gamma)  }
%\frac{\Gamma(\frac{n-1}{2})}{\left({\sum_{i=1}^n (\log(X_i-\gamma)- n^{-1} \sum_{j=1}^n \log(X_j - \gamma) )^2}\right)^{(n-1)/2}}
%\mathrm 1_{(X_{(1)} > \gamma)}.
%\end{align*}
%
%Suppose $X_i = \exp\{\mu_0 + \sigma_0 Z_i\} + \gamma_0$.
%Note that
%\begin{align*}
%     \prod_{i=1}^n (X_i-\gamma)
%     =&
%     \exp\{\sum_{i=1}^n \log (X_i - \gamma_0 + \gamma_0 -\gamma)\}
%     \\
%     =&
%     \exp\{
%         \sum_{i=1}^n \log (X_i - \gamma_0) + 
%     \sum_{i=1}^n \log (1+ \frac{\gamma_0 -\gamma}{X_i - \gamma_0 })\}
%     \\
%     =&
%     \exp\{
%         n\mu_0 + \sigma_0 \sum_{i=1}^n \log Z_i  + 
%     \sum_{i=1}^n \log (1+ \frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_i \} })\}
%     \}
%\end{align*}
%
%Let $R_n = \max_{1\leq i \leq n} |Z_i|$.
%Then $R_n \xrightarrow{P^n_{\theta_0}}\sqrt{2\log n}$.
%
%Thus, 
%\begin{align*}
%\left|
%\frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_i \} }
%\right|
%\leq
%\frac{
%\left|
%\gamma -\gamma_0
%\right|
%}{\exp\{\mu_0 - \sigma_0 R_n \} }.
%\end{align*}
%Hence if $|\gamma - \gamma_0 | = O(\exp\{ - (1+\epsilon) \sigma_0  \sqrt{2\log n}\})$, then
%\begin{align*}
%\frac{
%\left|
%\gamma -\gamma_0
...
%     =&
%     \exp\{
%         n\mu_0 + \sigma_0 \sum_{i=1}^n Z_i  + 
%     \sum_{i=1}^n \log (1+ \frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_i \} })\}
%     \\
%     =&
%     \exp\{
%         n\mu_0 + \sigma_0 \sum_{i=1}^n Z_i  + 
%     \sum_{i=1}^n  \frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_i \} }
%     -
%    (1 + o_{P_{\theta_0}^n }(1))
%     \frac 1 2 \sum_{i=1}^n  \frac{(\gamma_0 -\gamma)^2}{\exp\{2\mu_0 + 2\sigma_0 Z_i \} }
%     \}
%\end{align*}
%
%On the other hand,
%\begin{align*}
%    &
%{\sum_{i=1}^n \left(\log(X_i-\gamma)- n^{-1} \sum_{j=1}^n \log(X_j - \gamma) \right)^2}
%\\
%=&
%{\sum_{i=1}^n \left( \sigma_0 Z_i  -\sigma_0 \bar Z 
%+
%\log(1 +\frac{\gamma_0 - \gamma} {\exp(\mu_0 + \sigma_0 Z_i)})
%- n^{-1} \sum_{j=1}^n \log(1 +\frac{\gamma_0 - \gamma} {\exp(\mu_0 + \sigma_0 Z_j)}) \right)^2}
%= (1+o_{P_{\theta_0}^n}(1))n\sigma_0^2.
%\end{align*}
%
%%We have
%%\begin{align*}
...
%%     -
%%    (1 + o_{P_{\theta_0}^n }(1))
%%     \frac 1 2   \frac{(\gamma_0 -\gamma)^2}{\exp\{2\mu_0 + 2\sigma_0 Z_i \} }
%%\end{align*}
%
%%\begin{align*}
%%    &
...
%%      \frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_i \} }
%%- n^{-1} \sum_{j=1}^n 
%%      \frac{\gamma_0 -\gamma}{\exp\{\mu_0 + \sigma_0 Z_j \} }
%%     -
%%    (1 + o_{P_{\theta_0}^n }(1))
%%     \frac 1 2   \frac{(\gamma_0 -\gamma)^2}{\exp\{2\mu_0 + 2\sigma_0 Z_i \} }
%%+ n^{-1} \sum_{j=1}^n 
%%    (1 + o_{P_{\theta_0}^n }(1))
%%     \frac 1 2   \frac{(\gamma_0 -\gamma)^2}{\exp\{2\mu_0 + 2\sigma_0 Z_j \} }
%%\right)^2}
%%\end{align*}
...
%Here we take the variance of each variable to be $0.25$ since.
...
%$\pi(\bbeta)\sim \mathcal N (0, \BI_p)$, $\pi_0(\bbeta) \sim \mathcal N (0, \BI_{p_0})$.
%The LRT is implemented via Python module \emph{statsmodels} \cite{seabold-proc-scipy-2010}, version 0.12.0.
...
%If the MLE does not exist, the numerical optimization procedure may fail to converge and the resulting estimators of the parameters are, of course, unreliable.
%We note that when $\xi$ is largely deviated from $0$, the empirical power of the generalized FBF is precisely $0$ while the LRT is a little unstable.
%In fact, the implementation of the LRT relies on the numerical optimization.
...
%In fact, when $\xi$ is largely deviated from $0$, then the and the.
%It can be seen that the LRT has inflated empirical level, especially when the nuisance parameter $\nu$ is deviated from $(0,0)^\myT$.
...
%In comparison, the generalized FBF can reasonably preserve the test level although the test level is a little inflated in some cases.
%In comparison, the empirical level for the generalized FBF is more reasonable although it is also inflated.
%Also, it can be seen that when the generalized FBF and the LRT have similar empirical level, they also have similar empirical power.
\begin{table*}[ht]
    ...
    \begin{tabular}{llccccccccc}
        ...
        \\
        &$10$  &
        1.0000  & 0.4907 & 0.9998 & %0.9980  & 
        1.0000  & 0.4970 & 0.9998 & %0.9985  &
        1.0000  & 0.4802 & 0.9997  %0.9982 
        \\
        &$20$  &
        ...
        \\
        &$10$  &
        1.0000 & 0.9219 & 1.0000 & %0.9998  &
        1.0000 & 0.9254 & 1.0000 & %0.9997  &
        1.0000 & 0.9185 & 1.0000  %0.9998  
        \\
        &$20$  &
...
%        $50$ 
%        &$0$ (size) &
%        0.0574 & 0.0435 & 0.0573 &
%        0.0632 & 0.0386 & 0.0620&
%        0.0641 & 0.0423 & 0.0603 
%        \\
%        &$0.5$ &
%        0.4082 & 0.3629 & 0.4094 &
%        0.3509 & 0.3016 & 0.3462 &
%        0.3364 & 0.2675 & 0.3288
%        \\
%        &$1$ &
%        0.8863 & 0.8544 & 0.8859 &
%        0.8461 & 0.7975 & 0.8416 &
%        0.8059 & 0.7525 & 0.7991
%        \\
%        &$2$ &
%        0.9997 & 0.9994 & 0.9996 &
%        0.9994 & 0.9969 & 0.9993 &
%        0.9984 & 0.9948 & 0.9983
%        \\
%        &$5$ &
%        1.0000 & 0.9117 & 1.0000 &
%        1.0000 & 0.9047 & 1.0000 &
%        1.0000 & 0.8983 & 1.0000
%        \\
%        &$10$  &
%        1.0000  & 0.4907 & 1.0000 & %0.9980  & 
%        1.0000  & 0.4970 & 1.0000 & %0.9985  &
%        1.0000  & 0.4802 & 1.0000  %0.9982 
%        \\
%        &$20$  &
%        1.0000  & 0.1044 & 1.0000  & 
%        1.0000  & 0.1002 & 1.0000  &
%        1.0000  & 0.0976 & 1.0000 
%        \\
%        \midrule
%        $100$ 
%        &$0$ (size)  &
%        0.0519 & 0.0456 & 0.0536 &
%        0.0540 & 0.0449 & 0.0547 &
%        0.0566 & 0.0458 & 0.0563
%        \\
%        &$0.5$ &
%        0.6661 & 0.6426 & 0.6739 &
%        0.6110 & 0.5806 & 0.6118 &
%        0.5549 & 0.5228 & 0.5529
%        \\
%        &$1$  &
%        0.9940 & 0.9933 & 0.9946 &
%        0.9875 & 0.9838 & 0.9874 &
%        0.9798 & 0.9731 & 0.9791
%        \\
%        &$2$ &
%        1.0000 & 1.0000 & 1.0000 &
%        1.0000 & 1.0000 & 1.0000 &
...
%\begin{table*}[ht]
%    \caption{Empirical size and power of the LRT and the generalized FBF for logistic regression model. $n = 50$.}
%    \label{logistic_table1}
%    \centering
...
%        \\ 
%        \midrule
%        $(0,0)$ &0.0568&0.8826&1.0000&1.0000&0.0559&0.8819&1.0000&1.0000    \\
%        $(1,1)$ &0.0615&0.8012& & &0.0583&0.7958& &    \\
%        $(2,2)$ &0.0707&0.6607& & &0.0662&0.6455& &  \\
%        $(3,3)$ &0.0765&0.5423& & &0.0780&0.5275& &   \\
%        $(4,4)$ &0.0759&0.4627& & &0.0941&0.4541& &  \\
%        $(5,5)$ &0.0797&0.3912& & &0.1134&0.4036& &  \\
%        \bottomrule
%    \end{tabular}
%\end{table*}
%
%\begin{table*}[ht]
%    \caption{Empirical size and power of the LRT and the generalized FBF for logistic regression model. $n=100$.}
%    \label{logistic_table2}
%    \centering
%    \begin{tabular}{ccccccccc}
%        \toprule
%          & \multicolumn{4}{c}{generalized FBF} &\multicolumn{4}{c}{LRT}\\
...
%        \\ 
%        \midrule
%        $(0,0)$ & 0.0556 &  & 1.0000 & 1.0000 & 0.0568 &  & 1.0000 & 1.0000 \\
%        $(1,1)$ & 0.0533 &  & 1.0000 & 1.0000 & 0.0531 &  & 1.0000 & 1.0000 \\ 
%        $(2,2)$ & 0.0637 &  & 1.0000 & 1.0000 & 0.0578 &  & 1.0000 & 1.0000 \\
%        $(3,3)$ & 0.0647 &  &        & 1.0000 & 0.0579 &  &  & 1.0000\\
%        $(4,4)$ & 0.0686 &  &        &        & 0.0613 &  &        &        \\
%        $(5,5)$ & 0.0760 & & & &0.0683\\
%        \bottomrule
%    \end{tabular}
...
%Note that the MLE does not exist when the data points are completely separated or quasi-completely separated.

%Now we examine the performance of the generalized FBF by simulations.
%For comparison, we also carry out simulations for the LRT.
...
%its empirical power is consistently higher than the LRT.
\begin{table*}[ht]
...
%\begin{lemma}[\cite{Kleijn2012The}, Lemma 2.1.]\label{Thm:localExpansion}
    ...
    %\begin{equation*}
        %\sup_{\|h\|\leq M}\Big|
         %\log \frac{p_n(\BX^{n}|\theta_0+n^{-1/2}h)}{p_n(\BX^{n}|\theta_0)}-h^\myT  I_{\theta_0}\Delta_{n,\theta_0}+\frac{1}{2}h^\myT  I_{\theta_0}h
         %\Big|\xrightarrow{P^n_{\theta_0}}0.
    %\end{equation*}
...
\begin{proof}
    ...
    %Then $\rho (T_{m, g(m)}, 0) \to 0$ as $m \to \infty$.
    ...
    %In fact, for any given $m_0$, if $n \geq g(m_0)$, we have $h(n) \geq m_0$ by definition of $h$.
    %Consequently, $\rho(T_{h(n), n}, 0) \leq $.
...
\begin{equation*}
    ...
    %&=\left(
        ...
        %\frac{1}{\sqrt{n}}\sum^n_{i=1}\dot{\ell}_{\theta_0}(X_i)
        %\\
        %\frac{1}{\sqrt{n}}\sum^n_{i=1}h_n^\myT \dot{\ell}_{\theta_0}(X_i)-\frac{1}{2}h_n^\myT  I_{\theta_0}h_n
        %\end{matrix}
    %\right)
    %+o_{P_0^n}(1)\\
...
Thus, $L_t(\BX^n)$ is finite if and only if $ \gamma > p(t-1)/t$.
If $t>1$, $ p(t-1)/t > 0$, then $L_t(\BX^n) = + \infty$ provided $\gamma \in (0, p(t-1)/t]$.
This completes the proof.
\end{proof}
...
\end{split}
\end{equation}
    %So we only need to consider $\int_{\|h\|\leq M} \exp\big[h^\myT  I_{\theta_0}\Delta_{n,\theta_0}-\frac{1}{2}h^\myT  I_{\theta_0}h\big]\pi_n (h;\BX^{n}) \, dh$.
...
%the multivariate Berry-Esseen bound 
Note that
\begin{align*}
    &
    \sum_{i=1}^n
...
\begin{proposition}\label{lastProp}
    ...
    %If $M_n / \sqrt{{\log n}/(2(t\wedge (1-t)))} \to \infty$, then
...
%\begin{equation*}
%    \sup_{\omega\in [0,1], \xi \in \mathbb{R}}
...
%The following lemma shows that under Assumption \ref{Assumption:IBF}, 
 %$\BX^{\mathcal L}$ is minimal if and only if $\text{Card}(\mathcal L) = k$.
%\begin{lemma}
%    Under Assumption \ref{Assumption:IBF}, with $P_{\theta_0}^n$ probability $1$, we have
%    \begin{align*}
%        \mathscr L = \{\mathcal L \subset \{1,\dots,n\}: \text{Card}(\mathcal L) = k \}.
%    \end{align*}
%    \label{lemma:IBF_iff_k}
%\end{lemma}
