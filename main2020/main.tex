...
As a result, the Bayes factor does not have Wilks phenomenon in general, which hinders its application in frequentist hypothesis testing.
%k, and can not be directly used to construct frequentist test.
...
%the Bayes factor for point null hypothesis, and applied the result to construct frequentist tests.
%\cite{Johnson2013a}.
...
%It can be seen that the Lindley's paradox does not only exist when using improper priors.
    Theorem \ref{prop:expansion} implies that the asymptotic null distribution of the Bayes factor depends on the nuisance parameters and the priors.
...
Let $D_1(\theta_1 \| \theta_2) = \int_{\mathcal X} \log \left( p(x | \theta_0) / p( x | \theta) \right) p( x |\theta_0) \, \mathrm d \mu (x)$ be the Kullback-Leibler divergence between $P_{\theta_1}$ and $P_{\theta_2}$.
It is known that $\lim_{t \uparrow 1} D_t(\theta_1 \| \theta_2) = D_1 (\theta_1 \| \theta_2)$; see, e.g., \cite{Erven2014}, Theorem $5$.
Let 
$V(\theta_0 \| \theta) = \int_{\mathcal X} \left( \log \left( p(x|\theta_0) / p(x | \theta) \right) - D_1 (\theta_0 \| \theta) \right)^2  p( x |\theta_0) \, \mathrm d \mu (x)$.
...
\end{assumption}
    ...
    Then the first two conditions of Assumption \ref{assumption:prior} becomes
        %$\int_{\Theta} V(\theta_0 \|\theta) \exp\left\{ -c^\dagger D_{1}(\theta_0 \| \theta) \right\} \pi (\theta) \, \mathrm d \theta < \infty$ becomes
    \begin{align*}
...
\end{proposition}
\begin{remark}
    Most existing results on the consistency of posterior requires that $\phi(\theta)$ is proper.
...
\begin{equation*}
    \left(\frac{1}{\sqrt n}\sum_{i=1}^n X_i,\log \frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\right)^\top 
...
\begin{equation*}
    \left(\frac{1}{\sqrt{kn}}\sum_{i=1}^n (2X_i-k), \log \frac{p_n(\BX^{n}|\theta)}{p_n(\BX^{n}|\theta_0)}\right)^\top 
        \overset{P^n_{\theta_0}}{\rightsquigarrow}
        \mathcal{N}_2\left(
    \begin{pmatrix}
        0\\
