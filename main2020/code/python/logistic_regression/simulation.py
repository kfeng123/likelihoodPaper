...
class logistic_simulator():
    def __init__(self, n, p, p0, true_nu, true_xi, numerator_power = 2/3, denominator_power = 1/3, sampling_N = 80000, M = 10000):
    ...
    def do_one_simul(self):
        ...
        try:
            ...
            full_llf = sm_model_full.fit(method="newton").llf
        except:
            full_error = True
            full_llf = 0
        else:
        ...
        try:
            ...
            null_llf = sm_model_null.fit(method="newton").llf
        except:
            null_error = True
            null_llf = 0
        else:
...
if __name__ == "__main__" :
    import time
    now_time = time.time()
    simulator = logistic_simulator(n =20, p=3, p0=2, true_nu=[0] * 2, true_xi = [0] * 1)
    simulator.do_it()
